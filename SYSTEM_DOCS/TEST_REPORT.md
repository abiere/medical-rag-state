...
**Datum:** 14-04-2026 19:13:14  
**Duur:** 1.2s  
**Uitslag:** ✅ GESLAAGD  
**Score:** 16/16 geslaagd
  (8 overgeslagen, 0 mislukt, 0 fouten)
...
| ⏭️ SKIP | `test_qdrant_client_import` | qdrant_client kan worden geïmporteerd (vector store client) | 1 ms |
...
| ✅ PASS | `test_epub_parsing` | EPUB-bestand kan worden geparsed; hoofdstuktekst is terug te vinden | 169 ms |
| ⏭️ SKIP | `test_pdf_text_extraction` | PDF-tekst kan worden geëxtraheerd; page_number veld is aanwezig in resultaat | 76 ms |
...
| Status | Test | Beschrijving | Tijd |
|---|---|---|---|
| ⏭️ SKIP | `test_corrupt_pdf_detection` | Beschadigde PDF wordt afgehandeld zonder crash (graceful error handling) | 1 ms |
| ⏭️ SKIP | `test_figure_detection` | Docling detecteert een ingebedde afbeelding in een PDF | 0 ms |
| ✅ PASS | `test_page_count_extraction` | Paginatelling van een 3-pagina PDF klopt met de verwachte waarde van 3 | 126 ms |
...
| ✅ PASS | `test_web_dashboard_root` | FastAPI dashboard geeft HTML terug met 'Medical RAG', 'Qdrant' en 'Ollama' | 734 ms |
...
| `QualityTests.test_figure_detection` | docling niet geïnstalleerd |
